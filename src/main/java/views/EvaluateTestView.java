...
public class EvaluateTestView extends JPanel implements PropertyChangeListener {
  ...
  /**
   * Constructs an EvaluateTestView with the given view model.
   *
   * @param viewModel the view model for test evaluation
   */
  ...
  private void renderView() {
    ...
    String progressText = "Question " + (index + 1) + " / "
    ...
    switch (correctnessValue) {
      case "1":
        ...
        // Dark Green
        correctnessIndicator.setForeground(new Color(0, 150, 0));
        break;
      case "0.5":
        correctnessIndicator.setText("Partially Correct");
        // Orange
        correctnessIndicator.setForeground(new Color(255, 140, 0));
        break;
      case "0":
  ...
  /**
   * Creates a labeled scroll pane for a text area.
   *
  ...
  private JScrollPane createLabeledScrollPane(String labelText, JTextArea textArea) {
    ...
    // Returning scrollpane directly might be better if label is outside
    return scrollPane;
  }
